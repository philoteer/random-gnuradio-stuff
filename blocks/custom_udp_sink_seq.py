...
class blk(gr.sync_block):  # other base classes are basic_block, decim_block, interp_block
	...
	def __init__(self, server_ip = "192.168.0.1", server_port = 80):  # only default arguments here
		...
		self.pkt_max_len = 504
		self.pkt_max_len_with_seq = 508
		self.seq_no = np.dtype('uint32').type(0)
	...
	def work(self, input_items, output_items):
		...
		while ((cnt * self.pkt_max_len) < data_len): 
			self.sock.sendto(self.seq_no.tobytes() + data_tx[self.pkt_max_len *cnt:self.pkt_max_len *cnt+self.pkt_max_len],self.dest)
			cnt += 1
			self.seq_no += 1
		
		#probably not necessary anymore?
		if(len(data_tx[self.pkt_max_len *cnt:]) > 0):
			self.sock.sendto(self.seq_no.tobytes() + data_tx[self.pkt_max_len *cnt:],self.dest)			
			self.seq_no += 1
